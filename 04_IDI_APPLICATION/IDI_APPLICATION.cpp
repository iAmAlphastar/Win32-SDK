...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpszCmdLine, int iCmdShow)
{
	...
	wndclass.hIcon = LoadIcon(NULL, IDI_QUESTION);	//IDI_APPLICATION |IDI_HAND|IDI_QUESTION|IDI_EXCLAMATION|IDI_ASTERISK
	wndclass.hCursor = LoadCursor(NULL, IDC_ARROW);
	wndclass.hbrBackground = (HBRUSH)GetStockObject(BLACK_BRUSH);
	wndclass.lpszClassName = szAppName;
	wndclass.lpszMenuName = NULL;
	wndclass.hIconSm = LoadIcon(NULL, IDI_ASTERISK); 					//small icon (explorer/taskbar) newly added
	...
	ShowWindow(hwnd, SW_SHOWMINIMIZED); 	//hwnd - HANDLE to window ,iCmdShow is 4th parameter of WinMain()
	UpdateWindow(hwnd);
